  ...
  return (
    <section className="relative py-16 px-4 sm:px-6 max-w-3xl mx-auto">
      ...
      <div className={`relative border-l-2 ${
        theme.id === 'pure-sakura' ? 'border-stone-300' : theme.isDark ? 'border-pink-500/30' : 'border-pink-300'
          ...
          >
            ...
            <div className={`absolute -left-[17px] top-1.5 w-8 h-8 rounded-full ${
              theme.id === 'pure-sakura'
                ...
                : theme.isDark 
                ? 'bg-[#080c18] border-pink-400 shadow-pink-500/30' 
            ...
            <div className={`p-5 rounded-2xl border backdrop-blur-md space-y-1.5 ${
              theme.id === 'pure-sakura'
                ? 'bg-white/95 border-stone-300/80 text-stone-800 shadow-stone-200/50'
                : theme.isDark 
                ? 'bg-[#0f172a]/70 border-pink-500/20 text-zinc-300' 
                : 'bg-white/90 border-pink-200/80 text-zinc-800 shadow-pink-100/50'
            }`}>
              <h3 className={`font-serif text-lg font-bold ${
                theme.id === 'pure-sakura' ? 'text-stone-900' : theme.isDark ? 'text-white' : 'text-zinc-900'
              } tracking-wide`}>
                {item.title}
              </h3>
              <p className={`text-xs sm:text-sm ${
                theme.id === 'pure-sakura' ? 'text-stone-600' : theme.isDark ? 'text-zinc-300/90 font-light' : 'text-zinc-600 font-normal'
